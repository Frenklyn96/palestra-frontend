...
import { fetchEntrances,deleteEntranceAsync } from '../../features/slice/entrancesSlice';
...
const IngressiPage: React.FC = () => {
  ...
  const { entrances, totalCount, loading } = useSelector((state: RootState) => state.entrance);
  const userId = useSelector((state: RootState) => state.user.userId);

  const { results: resultsEntranceFiltered, totalCount: totalCountEntranceFIltered, } = useSelector((state: RootState) => state.generic);
  const { selectedCliente } = useSelector((state: RootState) => state.clienti);
  const [searchTerm, setSearchTerm] = useState('');
  const [openDeleteDialog, setOpenDeleteDialog] = useState(false);
  const [entranceToDelete, setEntranceToDelete] = useState<Entrance | null>(null);
  ...
  const [entranceToRender, setEntranceiToRender] = useState<Entrance[] | undefined>(undefined);
  ...
  const totalPages = Math.ceil((searchTerm ? totalCountEntranceFIltered : totalCount) / pageSize);
          ...
          <Box className="ingressi-header">
            ...
            <Box className="ingressi-actions">
              <GenericSearchTable
                ...
                userId={userId!}
                placeholder={t('ingressi_page.generic_search.search_placeholder')}
              />
             <Button variant="contained" startIcon={<AddIcon />} className='button-general' onClick={handleOpenAddDialog}>
            ...
            <Table>
              <TableHead>
                <TableRow>
                  ...
                  <TableCell onClick={() => handleSort('dataOra')} className="sortable-cell">
                ...
                {entranceToRender && entranceToRender.length > 0 ? (
                  entranceToRender.map((entrance, index) => (
                    ...
                    >
                      ...
                      <TableCell>{new Date(entrance.dataOra).toLocaleDateString('it-IT')}</TableCell>
                      <TableCell align="right">

                        <Tooltip title={t('ingressi_page.actions.delete')}>
                          <IconButton onClick={(e) => { e.stopPropagation(); handleDeleteClick(entrance); }}>
                            <DeleteIcon />
                          </IconButton>
                        </Tooltip>
                      </TableCell>
