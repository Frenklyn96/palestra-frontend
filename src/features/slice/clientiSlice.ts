...
export const getNumberMembersAsync = createAsyncThunk(
  ...
  async (id: string, thunkAPI) => {
    try {
      const number = await clienteApi.getNumberMembers(id);
      return number;
    } catch (error: any) {
...
const clientiSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder.addCase(fetchClienti.pending, (state) => {
      state.loading = true;
    });
    builder
