...
const IngressiDialog: React.FC<IngressiDialogProps> = ({ open, onClose }) => {
...
const [entrance, setEntrance] = useState<Entrance>({
  ...
  const handleSelectCliente = (id: string, nome: string, cognome: string) => {
    setEntrance(prev => ({ ...prev, clienteId: id, clienteName: `${nome} ${cognome}` }));
    setSearchTerm('');
    setFilteredClienti([]);
  };
    ...
    <Dialog open={open} onClose={onClose} maxWidth="sm" fullWidth>
      ...
      <DialogContent>
        {!entrance.clienteName ? (
          ...
          <Box className="form-row">
            <TextField
              label={t('ingressi_dialog.selected_cliente') || 'Cliente Selezionato'}
      ...
      <DialogActions className="dialog-actions">
        ...
        <Button onClick={handleSubmit} color="primary" disabled={!entrance.clienteId}>
