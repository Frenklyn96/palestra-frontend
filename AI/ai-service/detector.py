...
class PeopleCounter:
    def __init__(self):
        ...
        self.photos_dir = os.path.abspath(
            os.getenv("PHOTOS_DIR", os.path.join(os.path.expanduser("~"), "Pictures", "GymProjectAI", "photos"))
        )
    ...
    def get_line_y_ratio(self):
        """Ritorna l'altezza della riga corrente"""
        return self.line_y_ratio

    def set_line_y_ratio(self, new_ratio):
        """Imposta l'altezza della riga"""
        self.line_y_ratio = float(new_ratio)
        logger.info(f"Line Y ratio changed to {self.line_y_ratio}")
        return True
